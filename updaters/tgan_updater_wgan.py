    ...
    def update_core(self):
        ...
        if (self.it % self.freq) == 0:
            ...
            ret = [self.p.apply_async(
                ...
                (p.data.reshape(p.data.shape[0], -1).get(), name))    #.get() is causing problems
...
class TGANUpdaterWGANParallelSVCCond(TGANUpdaterBase):

    def __init__(self, *args, **kwargs):
        self.freq = kwargs.pop('freq') if 'freq' in kwargs else 1
        super(TGANUpdaterWGANParallelSVCCond, self).__init__(*args, **kwargs)
        self.p = Pool()
    ...
    def update_core(self):
        ...
        vgen_optimizer = self.get_optimizer('vgen')
        vdis_optimizer = self.get_optimizer('vdis')
        real_video, fake_video, dis_fake, dis_real = self.forward_cond()
