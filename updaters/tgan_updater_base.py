...
class TGANUpdaterBase(chainer.training.StandardUpdater):
    ...
    def _get_real_cond_video(self):
        xp = self.fsgen.xp
        batch = self.get_iterator('main').next()
        batch = xp.array(batch, dtype=np.float32).transpose(0, 2, 1, 3, 4)
        batch1 =np.expand_dims(batch[:,0,:,:,:],1)
        batch2 =np.expand_dims(batch[:,1,:,:,:],1)
    ...
    def _generate_fake_cond_video(self, z_slow, z_fast,cond):
        n_b, z_fast_dim, n_frames = z_fast.shape
        self.batchsize = n_b
        z_fast = F.reshape(F.transpose(
            z_fast, [0, 2, 1]), (n_b * n_frames, z_fast_dim))
    ...
    def forward_cond(self):
        real_video,cond = self._get_real_cond_video()
        batchsize = real_video.shape[0]
        z_slow, z_fast = self._get_seeds(batchsize)
        fake_video = self._generate_fake_cond_video(z_slow, z_fast,cond)
        ...
        fake = F.concat((fake_video,cond),axis=1)
        real = F.concat((real_video,cond),axis=1)
        dis_fake = self.vdis(fake)
