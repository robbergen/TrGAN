...
class TGANUpdaterBase(chainer.training.StandardUpdater):
    ...
    def _get_real_video(self):
        ...
        batch1 = np.expand_dims(batch[:, 0, :, :, :], 1) #Images
        batch2 = np.expand_dims(batch[:, 1, :, :, :], 1) #Conditions
        return Variable(batch1), Variable(batch2)
    ...
    #CONDITIONAL VERSION
    def _generate_fake_video(self, z_slow, z_fast,cond):
        n_b, z_fast_dim, n_frames = z_fast.shape
        self.batchsize = n_b
        z_fast = F.reshape(F.transpose(
            z_fast, [0, 2, 1]), (n_b * n_frames, z_fast_dim))
        ...
        with chainer.using_config('train', True):
            fake_video = self.vgen(z_slow, z_fast,cond)
            _, n_ch, h, w = fake_video.shape
    ...
    #CONDITIONAL VERSION
    def forward(self):
        real_video, cond = self._get_real_video()
        batchsize = real_video.shape[0]
        z_slow, z_fast = self._get_seeds(batchsize)
        fake_video = self._generate_fake_video(z_slow, z_fast, cond)
        # dis_out = self.vdis(F.concat([fake_video, real_video], axis=2))
        # dis_fake, dis_real = F.split_axis(dis_out, 2, 0)
        ...
        fake = F.concat((fake_video, cond), axis=1) #check axis
        real = F.concat((real_video, cond), axis=1)  # check axis
        dis_fake = self.vdis(fake)  #Need to concatenate variables here
        dis_real = self.vdis(real)
        return real_video, fake_video, dis_fake, dis_real
