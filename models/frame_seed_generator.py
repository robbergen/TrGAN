...
class FrameSeedGeneratorNoBetaInitUniform(chainer.Chain):
    ...
    def __init__(self, n_frames=16, z_slow_dim=256, z_fast_dim=256, wscale=0.01):
        ...
        w = chainer.initializers.Uniform(wscale)
        with self.init_scope():
            ############NEW VERSION BY ROB FOR 32 FRAMES####################
            self.dc0 = L.DeconvolutionND(1, z_slow_dim, 1024, 1, 1, 0, initialW=w)
            self.dc1 = L.DeconvolutionND(1, 1024, 512, 4, 2, 1, initialW=w)
            self.dc2 = L.DeconvolutionND(1, 512, 256, 4, 2, 1, initialW=w)
            self.dc3 = L.DeconvolutionND(1, 256, 128, 4, 2, 1, initialW=w)
            self.dc4 = L.DeconvolutionND(1, 128, 128, 4, 2, 1, initialW=w)
            self.dc5 = L.DeconvolutionND(1, 128, z_fast_dim, 4, 2, 1, initialW=w)
            self.bn0 = L.BatchNormalization(1024, use_beta=False)
            self.bn1 = L.BatchNormalization(512, use_beta=False)
            self.bn2 = L.BatchNormalization(256, use_beta=False)
            self.bn3 = L.BatchNormalization(128, use_beta=False)
            self.bn4 = L.BatchNormalization(128, use_beta=False)
            ##################################################################
            # self.dc0 = L.DeconvolutionND(1, z_slow_dim, 512, 1, 1, 0, initialW=w)
            # self.dc1 = L.DeconvolutionND(1, 512, 256, 4, 2, 1, initialW=w)
            # self.dc2 = L.DeconvolutionND(1, 256, 128, 4, 2, 1, initialW=w)
